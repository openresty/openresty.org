...
* now we require `gmake` (`Gnu make`) for `*BSD` systems even if [LuaJIT](luajit/) is not enabled. thanks [@lhmwzy](http://weibo.com/lhmwzy).
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc23.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc22.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc21.
...
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.37rc7.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc20.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc19.
    ...
    * bugfix: `lua_need_request_body` should not skip requests with methods other than `POST` and `PUT`. thanks [Nginx](nginx/) User.

#  Mainline Version 1.0.8.15 - 19 October 2011
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc17.
    ...
    * bugfix: fixed hanging issues when using `ngx.exec()` within `rewrite_by_lua` and `access_by_lua`. thanks [Nginx](nginx/) User for reporting it.

#  Mainline Version 1.0.8.13 - 16 October 2011
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc16.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc14.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc13.
    * now we implemented `ngx.req.set_uri()` and `ngx.req.set_uri_args()` to emulate `ngx_rewrite`'s `rewrite` directive (without `redirect` or `permanent` modifiers). thanks Vladimir Protasov (utros) and [Nginx](nginx/) User.
    * now we skip rewrite phase Lua handlers altogether if `ngx_rewrite`'s `rewrite` directive issue a location re-lookup by changing URIs (but not including rewrite ... break). thanks [Nginx](nginx/) User.
    * added constant `ngx.HTTP_METHOD_NOT_IMPLEMENTED` (501). thanks [Nginx](nginx/) User.

#  Mainline Version 1.0.8.7 - 15 October 2011
* upgraded [Srcache Nginx Module](srcache-nginx-module/) to 0.13rc2.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc11.
    * bugfix: now we explicitly clear all the modules' contexts before dump to named location with `ngx.exec`.
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.37rc6.
...
* applied the patch to the [Nginx](nginx/) core that always clears all modules' contexts in `ngx_http_named_location`.

#  Mainline Version 1.0.8.3 - 13 October 2011
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc10.
    * bugfix: calling `ngx.exec()` to jump to a named location did not clear the context object of [Lua Nginx Module](lua-nginx-module/) properly and might cause evil problems. thanks [Nginx](nginx/) User.
* upgraded [Iconv Nginx Module](iconv-nginx-module/) to 0.10rc5.
    * bugfix: fixed `-Wset-but-not-used` warnings issued by gcc 4.6.0. thanks 支家乐 (Calio).

#  Mainline Version 1.0.8.1 - 10 October 2011
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.37rc5.
    * bugfix: now we properly set the `Content-Length` request header for subrequests.
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc9.
...
* upgraded [Headers More Nginx Module](headers-more-nginx-module/) to 0.16rc3.
...
* upgraded [Srcache Nginx Module](srcache-nginx-module/) to 0.13rc1.
...
* upgraded [Rds Json Nginx Module](rds-json-nginx-module/) to 0.12rc6.
...
* upgraded [Drizzle Nginx Module](drizzle-nginx-module/) to 0.1.2rc2.
    * bugfix: fixed compatibility with nginx 1.1.4+
* upgraded [Memc Nginx Module](memc-nginx-module/) to 0.13rc1.
    * bugfix: fixed compatibility with nginx 1.1.4+.
* upgraded [Set Misc Nginx Module](set-misc-nginx-module/) to v0.22rc3.
    * minor code cleanup.
