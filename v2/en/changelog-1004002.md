<!---
    ...
    @description   Complete change log for OpenResty® 1.4.2, listing updates and fixes to the Nginx core, LuaJIT, and bundled components.
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.8.9.
    ...
    * bugfix: for SPDY requests, we (temporarily) disable the Lua API functions [ngx.location.capture](https:///wiki.nginx.org/HttpLuaModule#ngx.location.capture), [ngx.location.capture_multi]https:////wiki.nginx.org/HttpLuaModule#ngx.location.capture_multi), and [ngx.req.sockethttps://://wiki.nginx.org/HttpLuaModule#ngx.req.socket), which are known to have problems in SPDY mode. The SPDY compatibility issue will eventually get fixed in the near future.
