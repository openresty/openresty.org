<!---
    ...
    @description   Complete change log for OpenResty® 1.0.15, listing updates and fixes to the Nginx core, LuaJIT, and bundled components.
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.5.0rc32.
    * bugfix: [header_filter_by_lua](https://wiki.nginx.org/HttpLuaModule#header_filter_by_lua) and its friend would leak memory when it is the only directive of [Lua Nginx Module](lua-nginx-module.html) configured in a location and the [ngx.ctx](https://wiki.nginx.org/HttpLuaModule#ngx.ctx) API is also used there.
    ...
    * bugfix: the [ngx.arg](https://wiki.nginx.org/HttpLuaModule#ngx.arg) API was not usable within external Lua module files in [set_by_lua](https://wiki.nginx.org/HttpLuaModule#set_by_lua) or [set_by_lua_file](https://wiki.nginx.org/HttpLuaModule#set_by_lua_file).
    ...
    * bugfix: the Lua main thread stack might leak when [header_filter_by_lua](https://wiki.nginx.org/HttpLuaModule#header_filter_by_lua) or [header_filter_by_lua_file](https://wiki.nginx.org/HttpLuaModule#header_filter_by_lua_file) are used.
    * bugfix: the Lua VM might crash when calling the cosocket methods with a bad-typed `self` argument.
    * bufix: fixed the directive context for [set_by_lua](https://wiki.nginx.org/HttpLuaModule#set_by_lua) and [set_by_lua_file](https://wiki.nginx.org/HttpLuaModule#set_by_lua_file). they really work in the contexts `server`, `server if`, `location`, and `location if`. thanks Liu Taihua.
    * feature: added new directives [log_by_lua](https://wiki.nginx.org/HttpLuaModule#log_by_lua) and [log_by_lua_file](https://wiki.nginx.org/HttpLuaModule#log_by_lua_file) to support user Lua code running at the `log` request processing phase. thanks Matthieu Tourne for the patches.
    * feature: added new directives [body_filter_by_lua](https://wiki.nginx.org/HttpLuaModule#body_filter_by_lua) and [body_filter_by_lua_file](https://wiki.nginx.org/HttpLuaModule#body_filter_by_lua_file) to allow the user implement nginx output body filters in pure Lua.
    ...
    * feature: added new directive [lua_transform_underscores_in_response_headers](https://wiki.nginx.org/HttpLuaModule#lua_transform_underscores_in_response_headers). thanks Kindy Lin.
    * optimize: now we no longer register the `ndk` and `ngx` API for [set_by_lua](https://wiki.nginx.org/HttpLuaModule#set_by_lua)* and [header_filter_by_lua](https://wiki.nginx.org/HttpLuaModule#header_filter_by_lua)* at config time rather than request time. this makes these directives 200+% faster.
    * optimize: eliminated unnecessary string concatenations when generating tracebacks for Lua errors.
    * optimize: now we store the metatables for [ngx.req.socket()](https://wiki.nginx.org/HttpLuaModule#ngx.req.socket) and [ngx.socket.tcp()](https://wiki.nginx.org/HttpLuaModule#ngx.socket.tcp) objects into the Lua registry.
    * optimize: changed the string keys to light userdata for various tracking tables (coroutines table, regex cache table, cosocket connection pool table, [ngx.ctx](https://wiki.nginx.org/HttpLuaModule#ngx.ctx) table, and etc). This gives minor performance improvement from eliminating key hashing and etc. thanks Dirk Feytons.
    * diagnosis: now we issue user-friendly error messages when the [Nginx](nginx.html) Lua APIs are used in the wrong configure directive contexts (e.g., using [ngx.location.capture](https://wiki.nginx.org/HttpLuaModule#ngx.location.capture) in [set_by_lua](https://wiki.nginx.org/HttpLuaModule#set_by_lua)).
    * docs: fixed a typo: [ngx.now](https://wiki.nginx.org/HttpLuaModule#ngx.now) returns time of the resolution of milliseconds, rather than microseconds. thanks Wang Xi.
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.5.0rc30.
    * feature: new Lua API, [ngx.sleep()](https://wiki.nginx.org/HttpLuaModule#ngx.sleep), for doing non-blocking sleep in Lua. thanks jinglong for the patch.
    * feature: [ngx.log()](https://wiki.nginx.org/HttpLuaModule#ngx.log) now checks if the log level number is in the valid range (0 ~ 8). thanks Xiaoyu Chen (smallfish) for suggesting this improvement.
    * bugfix: [cosocket:receiveuntil](https://wiki.nginx.org/HttpLuaModule#tcpsock:receiveuntil) could leak memory, especially for long pattern string arguments. this bug was caught by [Test::Nginx::Socket](https://search.cpan.org/perldoc?Test::Nginx::Socket) when setting the environment `TEST_NGINX_CHECK_LEAK=1`.
    * bugfix: [ngx.re.sub()](https://wiki.nginx.org/HttpLuaModule#ngx.re.sub) could leak memory when the `replace` template argument string is not well-formed and the `o` regex option is also specified. this issue was caught by [Test::Nginx::Socket](https://search.cpan.org/perldoc?Test::Nginx::Socket) when setting environment `TEST_NGINX_CHECK_LEAK=1`.
    * bugfix: [ngx.re.gmatch](https://wiki.nginx.org/HttpLuaModule#ngx.re.gmatch) leaked memory when the `o` option was not specified. this bug was caught by [Test::Nginx::Socket](https://search.cpan.org/perldoc?Test::Nginx::Socket) when setting the environment `TEST_NGINX_CHECK_LEAK=1`.
    * bugfix: the Lua `_G` special table did not get cleared when [lua_code_cache](https://wiki.nginx.org/HttpLuaModule#lua_code_cache) is turned off. thanks Moven for reporting this issue.
    * bugfix: [cosocket:connect()](https://wiki.nginx.org/HttpLuaModule#tcpsock:connect) might hang on socket creation errors or segfault later due to left-over state flags.
...
https://mailman.nginx.org/pipermail/nginx-devel/2012-June/002328.html
...
fixed reads of uninitialized memory in the nginx core: https://mailman.nginx.org/pipermail/nginx-devel/2012-June/002281.html
...
fix a bug in the nginx core: https://mailman.nginx.org/pipermail/nginx-devel/2012-May/002231.html
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.5.0rc29.
    ...
    * bugfix: the [set_by_lua](https://wiki.nginx.org/HttpLuaModule#set_by_lua) directive did support nginx variable interpolation and there was no easy way to use the dollar sign characters in the literal Lua source. the [set_by_lua_file](https://wiki.nginx.org/HttpLuaModule#set_by_lua_file) directive still supports nginx variable interpolation in its lua file path argument. thanks Vittly for reporting this in [github issue #111](https://github.com/openresty/lua-nginx-module/issues/111) and jinglong for the test in [github pull #115](https://github.com/openresty/lua-nginx-module/pull/115).
    * bugfix: fixed compilation errors when PCRE is missing. thanks Dirk Feytons for the patch.
    * feature: added the `[lua]` prefix to the log messages produced by [ngx.log()](https://wiki.nginx.org/HttpLuaModule#ngx.log) and [print()](https://wiki.nginx.org/HttpLuaModule#print). thanks Matthieu Tourne for the patches.
    * feature: [ngx.log()](https://wiki.nginx.org/HttpLuaModule#ngx.log) and [print()](https://wiki.nginx.org/HttpLuaModule#print) now output more debugging info, i.e., the current lua source file name (if any), the current source line number, and the current calling Lua function name (if any), into the [Nginx](nginx.html) `error.log` file. An example message is 
```
[error] 56651#0: *1 [lua] test.lua:6: bar():
hello
```
. thanks Matthieu Tourne for the patch.
    * feature: added the [rewrite_by_lua_no_postpone](https://wiki.nginx.org/HttpLuaModule#rewrite_by_lua_no_postpone) directive which can control whether or not to disable postponing [rewrite_by_lua](https://wiki.nginx.org/HttpLuaModule#rewrite_by_lua) and [rewrite_by_lua_file](https://wiki.nginx.org/HttpLuaModule#rewrite_by_lua_file) to the end of the `access` request-processing phase. thanks Matthieu Tourne for the patches.
    * feature: added new Lua method [ngx.decode_args](https://wiki.nginx.org/HttpLuaModule#ngx.decode_args) to decode URI-encoded query strings into Lua tables. thanks Matthieu Tourne for the patches.
    * feature: the special `$prefix` and `${prefix} ` notations can now be used in the directives [lua_package_path](https://wiki.nginx.org/HttpLuaModule#lua_package_path) and [lua_package_cpath](https://wiki.nginx.org/HttpLuaModule#lua_package_cpath) to indicate the nginx `server prefix` path (usually determined by the `-p PATH` command-line option while starting the [Nginx](nginx.html) server. thanks Matthieu Tourne for the patches.
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.5.0rc28.
    * bugfix: [ngx.req.socket()](https://wiki.nginx.org/HttpLuaModule#ngx.req.socket) did not honor the `Expect: 100-continue` request header and could hang. thanks Matthieu Tourne for the patch in [pull request #107](https://github.com/openresty/lua-nginx-module/pull/107).
