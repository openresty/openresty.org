<!---
    ...
    @description   Complete change log for OpenResty® 1.0.9, listing updates and fixes to the Nginx core, LuaJIT, and bundled components.
...
in access log variable values. requested by [@姜大炮](https://weibo.com/egis).
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.3.1rc28.
    * bugfix: `Cache-Control` header modification might introduce empty value headers when using with the standard [ngx_headers](https://wiki.nginx.org/HttpHeadersModule) module.
...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.3.1rc27.
    * feature: added the [ngx.encode_args](https://wiki.nginx.org/HttpLuaModule#ngx.encode_args) method to encode a Lua code to a URI query string. thanks 郭颖 ([0597虾](https://weibo.com/shrimp0597)).
    * feature: [ngx.location.capture](https://wiki.nginx.org/HttpLuaModule#ngx.location.capture) and [ngx.exec](https://wiki.nginx.org/HttpLuaModule#ngx.exec) now supports the same Lua args table format as in [ngx.encode_args](https://wiki.nginx.org/HttpLuaModule#ngx.encode_args). thanks 郭颖 ([0597虾](https://weibo.com/shrimp0597)).
