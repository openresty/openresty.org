<!---
    ...
    @description   Complete change log for OpenResty® 1.4.3, listing updates and fixes to the Nginx core, LuaJIT, and bundled components.
    @creator       Yichun Zhang
    @created       2013-10-29 19:17 GMT
    @modifier      Yichun Zhang
    @modifier_link yichun-zhang
    @modified      2013-12-15 02:49 GMT
    @changes       123
--->
...
* upgraded [Redis Nginx Module](redis-nginx-module.html) module to 0.3.7.
    * see changes here: https://mailman.nginx.org/pipermail/nginx/2013-December/041297.html
...
* bugfix: applied the official patch [patch.2013.space.txt](https://nginx.org/download/patch.2013.space.txt) for
...
* upgraded the [Nginx](nginx.html) core to 1.4.3.
    * see the changes here: https://nginx.org/en/CHANGES-1.4
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.9.1.
    * feature: added the new configuration directive [lua_use_default_type](https://github.com/chaoslawful/lua-nginx-module#lua_use_default_type) for controlling whether to send out a default `Content-Type` response header (as defined by the [default_type](https://nginx.org/en/docs/http/ngx_http_core_module.html#default_type) directive). default on. thanks aviramc for the patch.
    ...
    * bugfix: [ngx.req.set_header("Host", value)](https://github.com/chaoslawful/lua-nginx-module#ngxreqset_header) would overwrite the value of [$host](https://nginx.org/en/docs/http/ngx_http_core_module.html#var_host) with bad values. thanks aviramc for the patch.
    ...
    * bugfix: reading [ngx.status](https://github.com/chaoslawful/lua-nginx-module#ngxstatus) might get different values than [$status](https://nginx.org/en/docs/http/ngx_http_core_module.html#var_status). thanks Kevin Burke for the report.
...
* upgraded [Headers More Nginx Module](headers-more-nginx-module.html) to 0.23.
    ...
    * bugfix: [more_set_input_headers](https://github.com/agentzh/headers-more-nginx-module#more_set_input_headers) might overwrite the value of the [$host](https://nginx.org/en/docs/http/ngx_http_core_module.html#var_host) variable with bad values.
...
* upgraded [Lua Resty Redis Library](lua-resty-redis-library.html) to 0.17.
    ...
    * optimize: use [LuaJIT](luajit.html) 2.1's new [table.new() primitive](https://repo.or.cz/w/luajit-2.0.git/commit/c8cfca055) to pre-allocate space for Lua tables.
* upgraded [Lua Resty Upload Library](lua-resty-upload-library.html) to 0.09.
    * bugfix: removed use of the [module()](https://www.lua.org/manual/5.1/manual.html#pdf-module) function to prevent bad side-effects.
    * optimize: Removed use of lua tables and [table.concat()](https://www.lua.org/manual/5.1/manual.html#pdf-table.concat) for simple one-line Lua string concatenations.
* upgraded [Lua Resty MySQL Library](lua-resty-mysql-library.html) to 0.14.
    * bugfix: avoided using Lua 5.1's [module()](https://www.lua.org/manual/5.1/manual.html#pdf-module) function for defining our Lua modules because it has bad side effects.
    ...
    * optimize: use [LuaJIT](luajit.html) v2.1's new [table.new()](https://repo.or.cz/w/luajit-2.0.git/commit/c8cfca055) API to optimize Lua table allocations. when table.new is missing, just fall back to the good old "{}" constructor. this gives 12% overall speed-up for a typical result set with 500 rows when [LuaJIT](luajit.html) 2.1 is used.
    * optimize: eliminated use of [table.insert()](https://www.lua.org/manual/5.1/manual.html#pdf-table.insert) because it is slower than "tb[#tb + 1] = val".
    * optimize: switched over to the multi-argument form of [string.char()](https://www.lua.org/manual/5.1/manual.html#pdf-string.char).
    * optimize: no longer use Lua tables and [table.concat()](https://www.lua.org/manual/5.1/manual.html#pdf-table.concat) to construct simple query strings.
* upgraded [Lua Resty Web Socket Library](lua-resty-web-socket-library.html) to 0.02.
    * optimize: use [LuaJIT](luajit.html) 2.1's [table.new()](https://repo.or.cz/w/luajit-2.0.git/commit/c8cfca055) to preallocate space for Lua tables, eliminating the overhead of Lua table rehash.
...
the [Nginx](nginx.html) core to fix request hang caused by the [ngx_gzip](https://nginx.org/en/docs/http/ngx_http_gzip_module.html) and
[ngx_gunzip](https://nginx.org/en/docs/http/ngx_http_gzip_module.html) modules
...
the [Nginx](nginx.html) core to fix the request hang when using [proxy_cache_lock](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_cache_lock) in
...
fix an issue in the [ngx_gzip module](https://nginx.org/en/docs/http/ngx_http_gzip_module.html):
