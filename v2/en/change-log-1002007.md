...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.7.21.
...
* upgraded [Encrypted Session Nginx Module](encrypted-session-nginx-module/) to 0.03.
    * refactor: fixed typos in the source code: replacing "3des" with "aes"; thanks Edgar Liu for reporting this issue.
* upgraded [Iconv Nginx Module](iconv-nginx-module/) to 0.10.
...
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.45.
...
* upgraded [Drizzle Nginx Module](drizzle-nginx-module/) to 0.1.5.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.7.20.
...
* upgraded [Lua Resty My SQLLibrary](lua-resty-mysql-library/) to 0.13.
    * bugfix: 64-bit integer values in the MySQL packets (like last insert ids) could not be properly parsed due to the lack of support for 64-bit integers in [LuaJIT](luajit/)'s standard "bit" module. thanks Azure Wang for the patch implementing a temporary workaround.
    * docs: various typo fixes from Tor Hveem and doledoletree.
* upgraded [Lua Resty Memcached Library](lua-resty-memcached-library/) to 0.11.
    * feature: added new method "touch" for the new Memcached command "touch". thanks merlin for the patch.
* updated the [upstream_truncation patch](https://raw.github.com/agentzh/ngx_openresty/master/patches/nginx-1.2.7-upstream_truncation.patch) for the [Nginx](nginx/) core.
...
* updated the [dtrace patch](https://raw.github.com/agentzh/ngx_openresty/master/patches/nginx-1.2.7-dtrace.patch) for the [Nginx](nginx/) core.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.7.18.
    ...
    * feature: implemented the [ngx.req.raw_header()](http://wiki.nginx.org/HttpLuaModule#ngx.req.raw_header) function for returning the original raw HTTP protocol header string received by [Nginx](nginx/). thanks Matthieu Tourne for requesting this.
...
* upgraded [Lua Resty Upload Library](lua-resty-upload-library/) to 0.08.
...
* feature: applied [the variables_in_redis_pass patch](https://raw.github.com/agentzh/ngx_openresty/master/patches/ngx_http_redis-0.3.6-variables_in_redis_pass.patch) to [Redis Nginx Module](redis-nginx-module/) 0.3.6 to allow use of [Nginx](nginx/) variables in the [redis_pass](http://wiki.nginx.org/HttpRedisModule#redis_pass) directive. thanks Diptamay Sanyal for requesting this feature.
* bugfix: applied Lanshun Zhou's [run_posted_requests_in_resolver patch](https://raw.github.com/agentzh/ngx_openresty/master/patches/nginx-1.2.7-run_posted_requests_in_resolver.patch) to the [Nginx](nginx/) core: http://mailman.nginx.org/pipermail/nginx-devel/2013-March/003476.html
* bugfix:  applied the official [hotfix #1 patch](http://luajit.org/download/v2.0.1_hotfix1.patch) for the bundled [LuaJIT](luajit/) 2.0.1.

#  Mainline Version 1.2.7.1 - 22 February 2013
* upgraded the [Nginx](nginx/) core to 1.2.7.
    * see http://nginx.org/en/CHANGES-1.2 for changes.
* upgraded [LuaJIT](luajit/) 2.0 to 2.0.1.
