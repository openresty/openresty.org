...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.4.1.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.4.1rc4.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.4.1rc3.
...
* upgraded [Drizzle Nginx Module](drizzle-nginx-module/) to v0.1.2rc6.
    * bugfix: fixed all the warnings from the `clang` static analyzer.
    * feature: allow use of the `DDEBUG` macro from the outside (via the `-D DDEBUG=1` cc opton).
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.38rc1, [Set Misc Nginx Module](set-misc-nginx-module/) to 0.22rc5, [Headers More Nginx Module](headers-more-nginx-module/) to 0.17rc1, and [Memc Nginx Module](memc-nginx-module/) to 0.13rc3, to allow use of the `DDEBUG` macro from the outside (via the `-D DDEBUG=1` cc opton).

#  Stable Release 1.0.10.44 - 16 January 2012
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.37.
* upgraded [Headers More Nginx Module](headers-more-nginx-module/) to 0.16.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.4.0.
* upgraded [Encrypted Session Nginx Module](encrypted-session-nginx-module/) to 0.02.
...
* upgraded [Upstream Keepalive Nginx Module](upstream-keepalive-nginx-module/) to 0.7.
...
* upgraded [Echo Nginx Module](echo-nginx-module/) to 0.37rc8.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to v0.3.1rc45.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to v0.3.1rc43.
    ...
    * bugfix: could not compile with [Nginx](nginx/) 1.1.12+ because [Nginx](nginx/) 1.1.12 changed its regex API. now we call PCRE API directly and require at least PCRE 8.21 for the PCRE JIT support in our `ngx.re` API (since PCRE 8.20 had a bug in its JIT engine that it did not honor `pcre_malloc` and `pcre_free` at all).

#  Mainline Version 1.0.10.35 - 30 December 2011
* upgraded [Headers More Nginx Module](headers-more-nginx-module/) to v0.16rc7.
...
* upgraded [LuaJIT](luajit/) to 2.0.0beta9.
    * changes: http://luajit.org/changes.html
* upgraded [Lua Nginx Module](lua-nginx-module/) to v0.3.1rc42.
    * bugfix: [ngx.req.set_header](http://wiki.nginx.org/HttpLuaModule#ngx.req.set_header) might cause invalid memory reads because [Nginx](nginx/) request header values must be null terminated. thanks Maxim Dounin.
    * bugfix: `ngx.var.VARIABLE` might evaluate to nil even if there is a valid value because the [Nginx](nginx/) variable value's `valid` flag might not be initialized properly. this bad had appeared in v0.3.1rc40.
* upgraded [Headers More Nginx Module](headers-more-nginx-module/) to v0.16rc6.
    * bugfix: the [more_set_input_headers](http://wiki.nginx.org/HttpHeadersMoreModule#more_set_headers) directive might cause invalid memory reads because [Nginx](nginx/) request header values must be null terminated. thanks Maxim Dounin.

#  Mainline Version 1.0.10.29 - 17 December 2011
* upgraded [Lua Nginx Module](lua-nginx-module/) to v0.3.1rc41.
...
* upgraded [Headers More Nginx Module](headers-more-nginx-module/) to 0.16rc5.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to v0.3.1rc40.
    ...
    * bugfix: `ngx.var.VARIABLE` did not evaluate to `nil` when the [Nginx](nginx/) variable's `valid` flag is `0`.
    * docs: various documentation improvements. thanks [Nginx](nginx/) User.
    * bugfix: there were various places where we did not check the pointer returned by the memory allocator.
* upgraded [Set Misc Nginx Module](set-misc-nginx-module/) to v0.22rc4.
...
* upgraded [Srcache Nginx Module](srcache-nginx-module/) to v0.13rc3.
    * bugfix: fixed a regression with [Xss Nginx Module](xss-nginx-module/) for cache hits. this bug had appeared in v0.13rc1. thanks [万珣新](http://weibo.com/liseen).
...
* upgraded [Xss Nginx Module](xss-nginx-module/) to v0.03rc8.
...
* upgraded [Rds Json Nginx Module](rds-json-nginx-module/) to v0.12rc7.
...
* upgraded [Xss Nginx Module](xss-nginx-module/) to v0.03rc7.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc38.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc37.
    ...
    * docs: massive documentation improvements. thanks [Nginx](nginx/) User.

#  Mainline Version 1.0.10.17 - 26 November 2011
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc36.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc35.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc34.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc33.
    ...
    * docs: massive documentation improvement done by [Nginx](nginx/) User.

#  Mainline Version 1.0.10.9 - 24 November 2011
* upgraded [Xss Nginx Module](xss-nginx-module/) to 0.03rc6.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc31.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc30.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.3.1rc29.
