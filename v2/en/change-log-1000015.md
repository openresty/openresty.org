...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc32.
    * bugfix: [header_filter_by_lua](http://wiki.nginx.org/HttpLuaModule#header_filter_by_lua) and its friend would leak memory when it is the only directive of [Lua Nginx Module](lua-nginx-module/) configured in a location and the [ngx.ctx](http://wiki.nginx.org/HttpLuaModule#ngx.ctx) API is also used there.
    ...
    * feature: added support for loading Lua/[LuaJIT](luajit/) raw bytecode files automatically in all those `*_by_lua_file` config directives. thanks jinglong and chaoslawful for the patches. benchmark results have shown that, for very big Lua-based web applications, bytecode loading gives an order of magnitude speedup at first requests that trigger the Lua code loading.
    ...
    * diagnosis: now we issue user-friendly error messages when the [Nginx](nginx/) Lua APIs are used in the wrong configure directive contexts (e.g., using [ngx.location.capture](http://wiki.nginx.org/HttpLuaModule#ngx.location.capture) in [set_by_lua](http://wiki.nginx.org/HttpLuaModule#set_by_lua)).
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc30.
    ...
    * docs: massive wording improvements from the [Nginx](nginx/) Wiki site. thanks Dayo.
* upgraded [Rds Json Nginx Module](rds-json-nginx-module/) to 0.12rc10.
    * bugfix: refactored on-demand handler registration. the old approach rewrites to static (global) variables at config-time, which could have potential problems with nginx config reloading via the `HUP` signal.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc29.
    * bugfix: `cosocket:receive(0)` was not allowed and would throw an error saying `0` was a bad pattern. thanks huang kun for reporting this issue. This issue at least made [Lua Resty Redis Library](lua-resty-redis-library/) reject reading 0-size values.
    ...
    * feature: [ngx.log()](http://wiki.nginx.org/HttpLuaModule#ngx.log) and [print()](http://wiki.nginx.org/HttpLuaModule#print) now output more debugging info, i.e., the current lua source file name (if any), the current source line number, and the current calling Lua function name (if any), into the [Nginx](nginx/) `error.log` file. An example message is 
```
...
* upgraded [Lua Resty Redis Library](lua-resty-redis-library/) to 0.09.
...
* upgraded [LuaJIT](luajit/) to 2.0.0beta10.
    * see changes here: http://luajit.org/changes.html
* feature: added the `--with-luajit-xcflags=FLAGS` option to `./configure` to add more C compiler options to [LuaJIT](luajit/)'s build system.
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc28.
...
* upgraded [Rds Csv Nginx Module](rds-csv-nginx-module/) to 0.05rc2.
...
* upgraded [Lua Resty String Library](lua-resty-string-library/) to 0.06.
    ...
    * feature: added new Lua module `resty.aes` that exposes the AES submodule of OpenSSL via [LuaJIT](luajit/) FFI. thanks Chase Colman for the patch.

#  Mainline Version 1.0.15.3 - 13 May 2012
* now we bundle Sergey A. Osokin's [Redis Nginx Module](redis-nginx-module/), 0.3.6, which is also enabled by default. thanks Zhu Feng for requesting this.
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc27.
...
* upgraded [Set Misc Nginx Module](set-misc-nginx-module/) to 0.22rc8.
...
* upgraded [Srcache Nginx Module](srcache-nginx-module/) to 0.13rc8.
...
* upgraded [Lua Redis Parser Library](lua-redis-parser-library/) to v0.09.
...
* upgraded [Lua Rds Parser Library](lua-rds-parser-library/) to 0.05.
    * feature: added `redis.parser._VERSION`.
    * bugfix: now we use Lua userdata to allocate memory used on the C side to prevent potential leaks caused by malloc/free, as discussed in [github issue #6](https://github.com/agentzh/lua-redis-parser/issues/6).
* upgraded [Lua Resty Memcached Library](lua-resty-memcached-library/) to 0.07.
...
* upgraded [Lua Resty Upload Library](lua-resty-upload-library/) to 0.03.
...
* upgraded the [Nginx](nginx/) core to 1.0.15.
...
* upgraded [Lua Nginx Module](lua-nginx-module/) to 0.5.0rc25.
    ...
    * feature: added publich C functions `ngx_http_lua_add_package_preload`, `ngx_http_lua_get_global_state`, and `ngx_http_lua_get_request` to help other [Nginx](nginx/) C modules expose new functionalities to [Lua Nginx Module](lua-nginx-module/). thanks Brian Akins for suggesting them in [github pull request #101](https://github.com/chaoslawful/lua-nginx-module/pull/101).
    * feature: made `ngx_http_lua_api.h` visible to other [Nginx](nginx/) modules by adding `src/api/` to the `CORE_INCS` config variable value in the config file. thanks Brian Akins for suggesting this in [github pull request #105](https://github.com/chaoslawful/lua-nginx-module/pull/105).
    * feature: add the `gdbinit` script to ease Lua user code debugging (Wang Xiaozhe).
    * optimize: various optimizations in cosocket's timeout handling. this gives about 2.5+% performance boost in some benchmarks using [Lua Resty Redis Library](lua-resty-redis-library/) and [Lua Resty My SQLLibrary](lua-resty-mysql-library/).
* upgraded [Rds Json Nginx Module](rds-json-nginx-module/) to v0.12rc8.
...
* upgraded [Set Misc Nginx Module](set-misc-nginx-module/) to 0.22rc7.
...
* upgraded [Drizzle Nginx Module](drizzle-nginx-module/) to 0.1.2rc7.
...
* upgraded [Lua Resty My SQLLibrary](lua-resty-mysql-library/) to 0.07.
