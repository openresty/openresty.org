...
while (<$in>) {
    ...
    $c += s! (\s) ( resty\.core\.shdict | ngx\.(?:re|process|errlog|semaphore|ssl(?:\.session)?|balancer|ocsp|resp|req|pipe) ) ( [\s,.:;?] ) !
            my ($pre, $txt, $post) = ($1, $2, $3);
            (my $file = $txt) =~ s{\.}{/}g;
            "$pre\[$txt](https://github.com/openresty/lua-resty-core/blob/master/lib/$file.md#readme)$post"
            !egx;

    $c += s! (\s) (opm|resty-cli|lua-cjson|lua-tablepool|lua-redis-parser|lua-resty-(?:core|memcached|mysql|redis|dns|lock|lrucache|websocket|upload|limit-traffic|signal|shell)) ( [\s,.:;?] ) !$1\[$2](https://github.com/openresty/$2#readme)$3!gxs;
    ...
    $c += s! (\s) ( thread\.exdata | table\.(?:isempty|isarray|nkeys) ) ( (?:\(\))? ) ( [\s,.:;?] ) !$1\[$2$3](https://github.com/openresty/luajit2)$4!gxs;
