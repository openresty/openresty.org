<!---
    @title         变更日志 1.2.3
    @description   OpenResty® 1.2.3 版本的完整变更日志，记录 Nginx 核心、LuaJIT 及各 lua-resty-* 组件的更新与修复。
    @creator       Yichun Zhang
    @created       2012-10-14 20:37 GMT
...
* upgraded [Redis2 Nginx Module](redis-2-nginx-module.html) to 0.09.
    * bugfix: directives [redis2_query](https://wiki.nginx.org/HttpRedis2Module#redis2_query), [redis2_literal_raw_query](https://wiki.nginx.org/HttpRedis2Module#redis2_literal_raw_query), and [redis2_raw_queries](https://wiki.nginx.org/HttpRedis2Module#redis2_raw_queries) could not be inherited automatically by the `location if` blocks, resulting in the "no redis2 query specified or the query is empty" error. thanks Tomasz Prus for the patch.
