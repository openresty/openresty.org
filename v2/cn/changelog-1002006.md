...
* upgraded [Lua Nginx Module](lua-nginx-module.html) to 0.7.14.
    * feature: implemented named subpattern support in [ngx.re.match](https://wiki.nginx.org/HttpLuaModule#ngx.re.match), [ngx.re.gmatch](https://wiki.nginx.org/HttpLuaModule#ngx.re.gmatch), [ngx.re.sub](https://wiki.nginx.org/HttpLuaModule#ngx.re.sub), and [ngx.re.gsub](https://wiki.nginx.org/HttpLuaModule#ngx.re.gsub); also added new regex option `D` to allow duplicate named subpattern names. thanks Ray Bejjani for the patch.
